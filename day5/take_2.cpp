...
class PairToParse {
public:
  PairToParse(std::pair<Tp,Tp>&& pair) : m_pair{std::move(pair)} {};
  PairToParse(Tp&& left,Tp&& right) : m_pair{std::move(left),std::move(right)} {};
  auto left() {return m_pair.first;}
  auto right() {return m_pair.second;}
  operator std::string() const {return std::string{"<"} + std::string{m_pair.first} + "," + std::string{m_pair.second} + ">";}
private:
  std::pair<Tp,Tp> m_pair;
};

template <class Tp>
class ListToParse {
public:
  // TODO: Construct from intialiser list?
private:
  std::vector<Tp> m_list;
};

class ToParse {
public:  
  ToParse(std::string const& s={}) : m_s{s} {}

  auto to_pair(std::string delim) {
    auto begin = m_s.begin();
    auto end = m_s.end();
    auto iter = std::search(begin,end,delim.begin(),delim.end());
    if (iter != end) {
      return PairToParse<ToParse>{
         ToParse{std::string{begin,iter}}
        ,ToParse{std::string{iter+delim.size(),end}}};
    }
    else {
      return PairToParse<ToParse>{
         ToParse{std::string{begin,iter}}
...
Model parse(auto& in) {
    ...
    {
      // Make an attempt to create and use some chainable parsers.
      auto stacks_moves_split_result = ToParse{std::string{std::istreambuf_iterator<char>{in},std::istreambuf_iterator<char>{}}}
        .to_pair("\n\n");
      std::cout << "\nstacks_moves_split_result:" << std::string{stacks_moves_split_result};
      auto stacks_parse_result = stacks_moves_split_result
        .left()
        .to_lines(); 
      return result;
    }
    while (std::getline(in,line)) {
      switch (state) {
        case 0: {
          if (line.size()==0) {
            // 1. pop indices line and parse it into a map from index to empty Stacks
