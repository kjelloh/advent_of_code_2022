...
namespace did_not_use {
  ...
  public:
    ...
    Vertices const& adj(Vertex v) const {return m_adj.at(v);}
  ...
  private:
    using AdjecencyList = std::map<Vertex,Vertices>;
    Result m_E{};
    AdjecencyList m_adj{};
  };
  ...
  public:
    using Vertex = Vector;
    int V() const {return m_graph.V();}
    int E() const {return m_graph.E();}
    void add_edge(Vertex v,Vertex w) {
      auto int_v = to_graph_vertex(v);
      auto int_w = to_graph_vertex(w);
    }

  private:
    using Dict = std::vector<std::pair<Vertex,Graph::Vertex>>;
    Graph m_graph{};
    Dict m_dict{};
    Graph::Vertex to_graph_vertex(Vertex v) {
      auto iter = std::find_if(m_dict.begin(),m_dict.end(),[&v](auto const& entry){
        return entry.first == v;
      });
      if (iter == m_dict.end()) {
        iter = m_dict.insert(iter,{v,m_dict.size()});
      }
      return iter->second;
    }
  };
...
public:
  ...
  std::optional<Vector> to_pos(Vertex v) const {
    ...
    auto iter = std::find_if(m_map.begin(),m_map.end(),[&v](auto const& entry){
      return entry.second == v;
    });
    if (iter != m_map.end()) result = iter->first;
    return result;
  }
...
using Droplet = Vector;
using Droplets = std::vector<Droplet>;

using Model = Droplets;

std::ostream& operator<<(std::ostream& os,Droplets const& droplets) {
...
Droplet to_droplet(std::string const& s) {
  auto coords = to_tokens(s,",");
  return Droplet{.x=std::stoi(coords[0]),.y=std::stoi(coords[1]),.z=std::stoi(coords[2])};
}
...
Model parse(auto& in) {
    ...
    while (std::getline(in,line)) {
        result.push_back(to_droplet(line));
    }
    return result;
}
